...
module.exports.login = asyncHandler(async (req, res, next) => {
    ...
    if (!email || !password) {
        throw new CustomError("Some required fields are missing, both email and password is required!", 400);
    }
    ...
    // Check: wether user enters the correct password or not.
...
module.exports.isAuthenticated = asyncHandler(async(req, res, next) => {
    ...
    if(!token)
    {
        throw new CustomError("You are not logged in! Please log in to access the application.", 401);
    }
    ...
    if(!user)
    {
        throw new CustomError("The user belonging to this token no longer exists. Log in instead!", 401);
    }
